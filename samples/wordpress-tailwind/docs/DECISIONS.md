...
| 11 | Storybook | **Incluso** (`@storybook/html-vite` + twig.js runtime come area-broker, [ADR 0004](adr/0004-storybook-twigjs.md)) | Component library navigabile con controls/dark mode/addon a11y sullo stesso motore Twig della build (`twig ^1.17` pinnato, nessun drift di sintassi); superset della docs page | Storybook escluso (Fase 4); docs page generata al suo posto |
| 12 | Showcase | Storybook (`Base/*`, `Blocks/*`, `Forms/*`), una story per scenario mock, `play` per gli interattivi | QA visiva e stress test di dismiss/focus con controls nativi; `src/docs/` rimosso, mai copiato nel host theme | Docs page unica generata (Fase 5); una pagina per componente |
