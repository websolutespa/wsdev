...
| 7 | Token: sorgente e pipeline | Skill `figma-tokens` di un progetto interno precedente (generalizzata) con `use_figma` per leggere tutte le variabili in una volta; export DTCG **git-ignored** | Il codice è la source of truth; export rigenerabile | Export committato |
...
| 11 | Storybook | **Incluso** (`@storybook/html-vite` + twig.js runtime come nel progetto di riferimento, [ADR 0004](adr/0004-storybook-twigjs.md)) | Component library navigabile con controls/dark mode/addon a11y sullo stesso motore Twig della build (`twig ^1.17` pinnato, nessun drift di sintassi); superset della docs page | Storybook escluso (Fase 4); docs page generata al suo posto |
| 12 | Showcase | Storybook (`Styleguide/*`, `Base/*`, `Blocks/*`, `Layout/*`): `Styleguide/*` una pagina per famiglia di token; ogni componente `Default` + `play` per gli interattivi + `Catalog` (griglie varianti × taglie × stati) | QA visiva e stress test di dismiss/focus con controls nativi; `Catalog` per componente sostituisce l'elenco piatto "una story per scenario mock" della Fase 7; `src/docs/` rimosso, mai copiato nel host theme | Docs page unica generata (Fase 5); una pagina per componente senza Catalog/Styleguide |
...
| 17 | Moduli JS | **Copiare e auditare** i moduli del progetto interno precedente (comportamento, non brand); modello scelto per componente | Comportamento collaudato in produzione | Riscrivere da zero |
