...
| `<name>.stories.js` | always — generated by `npm run scaffold:stories -- <name>`, refined by hand (`play` only for JS-only states); see [Story shape](#story-shape) below |
...
Every `Base/*` and `Blocks/*` story follows the same shape: a Controls
playground (`Default`), an interactive `play` story only when the component
has a state reachable exclusively through JS (see below), and a `Catalog`
export that stacks curated
...
Every story exported as `Default` is wrapped automatically by a decorator in
`.storybook/preview.ts` in the same card shell as the `Catalog` cards (title =
component name, fullscreen layouts get an outer `p-4`); do not wrap it by hand.
Opt out with `parameters: { playgroundCard: false }`.
...
Modules are already live in every story: the global decorator in
`.storybook/preview.ts` runs `initModules` after each render, so a `play` that
only calls `initModules` is redundant and must not be written.

Add a `play` story only when the state it reaches cannot be expressed through
the component's params: top-layer or floating overlays (dialog, sheet, drawer,
popover, tooltip, hover-card, menus, select, combobox, date-picker), toasts,
live filtering. If a param expresses the state (`items[].open`, `pressed`,
`value`, the active tab, a collapsed flag), show it statically in the
`Catalog` instead. The `play` imports `initModules` from `~sb/modules` and
drives the real DOM trigger the module listens for (read the trigger from the
...
| button-group `[&>*…]` join rules reaching a Radix-`asChild` trigger | the same rules repeated as `[&>[data-module]…>[data-slot$=-trigger]]` | CSS only: select, dropdown-menu, popover and tooltip wrap their trigger in the module root, so the trigger is not a direct child of the group |
...
5. Module (if any) returns a working `dispose`; ARIA per APG; full keyboard
   support.
6. `<name>.stories.js` present and refined (`play` only for JS-only states).
