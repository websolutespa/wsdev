...
function isTest() {
  ...
  const packagePath = getPackagePath();
  const packagePathname = packagePath.pathname.replace(
    /^\/(\w\:\/)/,
    (a, b) => b
  );
...
function debug(name2, data) {
  if (DEBUG) {
    ...
    fs2.writeFileSync(
      pathname,
      JSON.stringify(data, replaceCircularReference(), 2),
      { encoding: 'utf8' }
    );
  }
}
...
async function checkUpdate() {
  try {
    ...
    if (response?.latest) {
      console.log();
      console.log(
        chalk2.gray('A new version of ') +
          getName() +
          chalk2.gray(' is available!')
      );
      console.log(
        chalk2.gray('Update by running ') + 'npm i -g @websolutespa/ws-cli'
      );
      console.log();
    }
  } catch (error2) {}
...
async function fsReadFiles(folderName) {
  try {
    const dirents = await fs3.promises.readdir(folderName, {
      withFileTypes: true,
    });
...
async function updateRepo(options, folder) {
  ...
  const sources = files.filter((x) =>
    /^\.(js|jsx|ts|tsx|mdx)$/.test(path3.extname(x))
  );
...
function updateScripts(options, data) {
  ...
  Object.entries(data).forEach(([key, value]) => {
    scripts[key] = String(value).replace(
      new RegExp(`@${options.sampleType}`, 'g'),
      `@${options.projectName}`
    );
  });
  return scripts;
...
function updateDependencies(options, data, isPackage) {
  ...
  Object.entries(data).forEach(([key, value]) => {
    if (key.indexOf('@websolutespa') !== -1) {
      ...
      dependencies[
        key.replace(`@${options.sampleType}`, `@${options.projectName}`)
      ] = String(value);
    } else {
...
async function updatePackage(options, pathname, isPackage) {
  try {
    ...
    if (data.name === options.sampleType) {
      ...
      data.name = data.name
        .split(`@${options.sampleType}`)
        .join(`@${options.projectName}`);
    }
    if (data.scripts) {
      data.scripts = updateScripts(options, data.scripts);
    }
    if (data.dependencies) {
      data.dependencies = updateDependencies(
        options,
        data.dependencies,
        isPackage
      );
    ...
    if (data.peerDependencies) {
      data.peerDependencies = updateDependencies(
        options,
    ...
    if (data.devDependencies) {
      data.devDependencies = updateDependencies(
        options,
...
async function updateSource(options, pathname) {
  try {
    const data = await fsRead(pathname);
    const source = String(data).replace(
      new RegExp(`@${options.sampleType}`, 'g'),
      `@${options.projectName}`
    );
...
async function updateJson(options, pathname) {
  try {
    ...
    if (typeof data.$schema === 'string') {
      data.$schema =
        data.$schema.indexOf('../../../node_modules/') === 0
...
async function getApp(app) {
  ...
  while (parentNode) {
    dependencies.push(
      ...parentNode.dependencies.map((x) =>
        path4.join(path4.relative(root, x), '*').split(path4.sep).join('/')
      )
    );
...
async function RunCommand() {
  return await yargs(hideBin(process.argv))
    ...
    .fail(async function (message, error2, yargs2) {
    ...
    .alias('h', 'help')
    .epilog(
      chalk3.gray(`websolute \xA9 ${/* @__PURE__ */ new Date().getFullYear()}
    `)
    )
    .parse();
}
